...
function FilterSidebar() {
    ...
    // Create updated filters with the new price
    const newFilters = {...filters, minPrice:0, maxPrice:newPrice};
    setFilters(newFilters);
    
    // Pass newFilters instead of undefined 'params'
    updateURLParams(newFilters); // FIXED: Changed params to newFilters
};
        ...
        <div className="p-4">
            ...
            <div className="mb-6">
                ...
                {categories.map((category) => (
                    <div key={category} className="flex items-center mb-1">
                        <input type="radio" name="category" value={category} onChange={handleFilterChange} checked={filters.category === category} className="mr-2 h-4 w-4 text-blue-600 focus:ring-blue-400 border-gray-300" />
            ...
            <div className="mb-6">
                ...
                {genders.map((gender) => (
                    <div key={gender} className="flex items-center mb-1">
                        <input type="radio" name="gender" value={gender} onChange={handleFilterChange} checked={filters.gender === gender}className="mr-2 h-4 w-4 text-blue-600 focus:ring-blue-400 border-gray-300" />
            ...
            <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                    {colors.map((color) => (
                        <button key={color} name="color" value={color} onClick={handleFilterChange} checked={filters.color === color} className="w-8 h-8 rounded-full border border-gray-300 transition hover:scale-105" style={{backgroundColor: color.toLowerCase()}}></button>
                    ))}
                </div>
            </div>
